<template>
  <view class="range-slider" :style="'width:' + width + 'rpx;height:' + height + 'rpx'">
    ...
    <view class="block" :class="{ active: isMinActive }"
      :style="'width:' + blockSize + 'rpx;height:' + blockSize + 'rpx;margin-left:' + minBlockLeft + 'rpx;'"
      @touchstart="_onBlockTouchStart" @touchmove.stop="_onBlockTouchMove" @touchend="_onBlockTouchEnd"
      :data-left="minBlockLeft" data-tag="minBlock">
      <!-- <slot name="minBlock"></slot> -->
      <view class="range-slider-block" :style="isMinActive?'display:block;':'display:none;'">{{blockMinVal}}</view>
    </view>
    <view class="block" :class="{ active: isMaxActive }"
      ...
      <view class="range-slider-block" :style="isMaxActive?'display:block;':'display:none;'">{{blockMaxVal}}</view>
    </view>
  </view>
</template>
<script>
    ...
    methods: {
      ...
      _onBlockTouchMove: function(e) {
        ...
        if (tag == 'minBlock' || tag == 'maxBlock') {
          ...
          this.blockMinVal = eventDetail.minValue;
          this.blockMaxVal = eventDetail.maxValue;
          var eventOption = {};
          //
      ...
      _refresh: function() {
        ...
        if (this.blockMinVal && this.blockMinVal) {
          values = [this.blockMinVal || values[0], this.blockMaxVal || values[1]];
        }
...
<style>
  ...
  .range-slider-block::after {
    ...
    bottom: -26rpx;
    left: 38rpx;
    border-color: #E3F0EA transparent transparent;
  }
</style>
