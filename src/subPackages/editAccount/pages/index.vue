<template>
  <view class="editAccount">
    ...
    <view class="account-top">
      <image
        class="bg"
        :src="`https://res-tasaki.baozun.com/static/images/account/account-bg2.png`" mode="widthFix">
      </image>
    ...
    <view class="edit-icon">
      <text class="icon-font icon-icon-shezhi"></text>
      编辑资料
    </view>
    <view class="form-box">
      <form @submit="submitBind">
        <view class="form-inner">
          ...
          <view class="input-box">
            <input v-model="formData.surnname" name="surnname" placeholder="姓氏" />
            <!-- <view class="errorMessage" v-if="rules.isSurnname">*请填写正确的姓氏</view> -->
          </view>
          <view class="input-box">
            <input v-model="formData.name"  name="name" placeholder="名字" />
            <!-- <view class="errorMessage" v-if="rules.isName">*请填写正确的名字</view> -->
          </view>
          <view class="input-box">
            <picker mode="date" :disabled="isChangeDate" :value="date" :start="startDate" :end="endDate" @change="bindDateChange">
              <view class="picker uni-input">
                <text class="date-placeholder" v-if="date == null">生日</text>
                <text class="date-text" v-else>{{date}}</text>
                <text class="icon-font icon-icon-xia"></text>
              </view>
            </picker>
            <view class="errorMessage" v-if="rules.isDate">*请选择生日</view>
          </view>
          <view class="input-box">
          ...
          <view class="input-box">
            <input v-model="formData.email" name="email" placeholder="电子邮箱（选填）" />
...
export default {
  ...
  methods: {
    ...
    // 获取头像
    getAvatarUrl() {
      const { avatarUrl, nickName } = uni.getStorageSync('weixinInfo') || {};
