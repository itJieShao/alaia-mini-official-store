<template>
  <view>
    <custom-nav-bar left-arrow="left" :head-border="false" />
    <view :style="{ 'padding-top': ktxStatusHeight }"></view>
    <navigator hover-class="none" url="/subPackages/search/pages/index">
      <search-input :disabled="true" />
    </navigator>
    <block v-if="pageShow">
      <search-res v-if="goodsList.length" :totalCount="totalCount" :keyWord="keyWord" :filterKeyWord="filterKeyWord"
        :img="img" :goodsList="goodsList" @updateList="updateList" @goFilter="goFilter" @scrollToTop="scrollToTop"
        :goTopFlag="goTopFlag" />
