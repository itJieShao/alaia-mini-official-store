<template>
  <view class="address-container">
    ...
    <template v-for="(item,index) in addressList">
      <view class="address-list" :key="index">
        <view class="address-left" v-if="code" @click="checkboxChange(item)">
          <text
            :class="item.node.isSelect ? 'active' : ''"
          ></text>
        </view>
        <view class="address-center">
          <view @click="checkboxChange(item)">
            <view class="name">
              ...
              <text>{{item.node.receiverMobile}}</text>
            </view>
          ...
          <view class="handle" v-if="!code">
            <text @click="deleteAddress(item.node.code)">移除</text>
            <text @click="goEditPage(item.node.code)">编辑</text>
          </view>
        </view>
        <view class="address-right" v-if="code">
          <text @click="goEditPage(item.node.code)">编辑</text>
        </view>
      </view>
    </template>
    <view class="add" @click="goNextPage('/subPackages/address/pages/add/index')">
      <view>添加新地址</view>
    </view>
  ...
  methods: {
        ...
        if (this.source == 'checkoutPage' && this.code) {
          const filterList = initList.map((item) => {
            if (item.node.code == this.code) {
              return { ...item.node, isSelect: true }
            }
            return { ...item.node, isSelect: false }
          })
          this.addressList = filterList.map((item) => ({ node: item }))
        } else {
    ...
    checkboxChange(item) {
      if (this.code) {
        ...
        this.addressList = list.map((item) => ({ node: item }))
