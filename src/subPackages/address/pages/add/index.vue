<template>
  <view class="address-add">
    <custom-nav-bar :has-left-radius="true" :title="code?'编辑地址':'添加新地址'" :head-font-color="false" />
      ...
      <form @submit="formSubmit" @reset="formReset">
        ...
        <view class="check-box">
          <view class="icon" @click="checkboxChange(item)">
            <view class="icon-act" v-if="formData.defaultAddress"></view>
          </view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleChange(e) {
        this.formData.area =
          `${this.regionsList[this.valueIndex[0]].name}/${this.regionsList[this.valueIndex[0]].children[this.valueIndex[1]].name}/${this.regionsList[this.valueIndex[0]].children[this.valueIndex[1]].children[this.valueIndex[2]].name}`
      },
      // 是否是默认地址
      checkboxChange(e) {
      ...
      async formSubmit(e) {
        ...
        const params = {
          ...
          districtId: this.regionsList[this.valueIndex[0]].children[this.valueIndex[1]].children[this.valueIndex[2]]
            .code,
          district: this.regionsList[this.valueIndex[0]].children[this.valueIndex[1]].children[this.valueIndex[2]]
            .name,
          address,
