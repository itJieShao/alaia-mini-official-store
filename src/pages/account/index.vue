  ...
  <scroll-view class="scroll-container" :style="{'padding-top':ktxStatusHeight}" :scroll-y="true">
    ...
    <view class="container">
      ...
      <view class="account-header" @click="goNextPage('/subPackages/editAccount/pages/index')">
        <!-- 背景图片 -->
        <image class="account-bg" :src="`https://res-tasaki.baozun.com/static/images/account/account-bg2.png`" mode="widthFix" :lazy-load="true"/>
        <!-- 头像 -->
      ...
      <view class="login-entrance">
        ...
        <view class="editer-info-btn" @click="goNextPage('/subPackages/editAccount/pages/index')">
      ...
      <view class="account-content">
        <view class="account-entrance-box">
          <view class="accout-entrance-item"  v-for="(accountItem,index) in accountList" :key="index" @click="goNextPage(accountItem.path)">
...
import { USER_INFO, SERVICE_TEL } from '../../constants/user'
...
export default {
  ...
  onShow() {
    ...
    this.getUserInfo().then((res) => {
      const { accountInfo } = uni.getStorageSync(USER_INFO) || {};
